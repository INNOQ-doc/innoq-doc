...
```mermaid
flowchart TD
    User[🧑 User] --> Agent[🤖 Swiss News Agent]
    Agent --> Decision{Which function?}
    Decision --> Headlines[getTopHeadlines 📈]
    Decision --> Search[searchNews 🔍]
    Headlines & Search --> API[(GNews API)]
    API --> Response[(JSON Response)]
    Response --> Output[📰 Friendly News Summary]
