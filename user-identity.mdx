...
<Warning>
  **Only Valid JSON Data Accepted** - Please ensure that all data provided for storage is in valid JSON format. 
  Non-JSON or invalid JSON data will be automatically ignored and not stored in the conversation history.
</Warning>
...
## Step 4: Leads Capturing

By enabling this feature, you can further utilize the data collected through the User Identity Verification process. 
This functionality allows for the conversion of saved user data into actionable leads.

All data saved from successful user verifications is automatically stored as a "Lead" in the system.

These leads can be accessed and managed through the "Leads" table in your platform. 

<Warning>
 **Specific JSON Data Requirements for Leads Capturing** - Please note that for the Leads Capturing feature, 
 only JSON data with specific fields containing string or number values is accepted. JSON data containing 
 boolean values or other data types will be ignored for lead conversion and instead stored as regular conversation data.
</Warning>
...
## Step 5: Inserting Your Token into GPT-Trainer Chatbot
