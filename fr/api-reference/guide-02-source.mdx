...
In this guide, we will walk through the process of uploading data sources to your chatbot using the INNOCHAT API.
...
The API endpoint for uploading data sources to your chatbot is: `https://app.innochat.ch/api/v1/chatbot/{uuid}/data-source/url`.
...
```bash Curl
curl --location --request POST 'https://app.innochat.ch/api/v1/chatbot/{uuid}/data-source/url' \
...
url = 'https://app.innochat.ch/api/v1/chatbot/{uuid}/data-source/url'
headers = {
...
const url = 'https://app.innochat.ch/api/v1/chatbot/{uuid}/data-source/url';
const headers = {
    'Content-Type': 'application/json',
    'Authorization': 'Bearer <token>'
};
...
That's it! You've now learned how to upload data sources to your chatbot using the innoChat API.
