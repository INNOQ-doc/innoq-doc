...
This is where function-calling comes in. Through function-calling, you can serve data on-demand to your AI Agent in GPT-trainer during a live 
...
<Note>
Note: it is ALWAYS advisable to write your own script and test for the output of the API endpoint first. You want to know exactly what kind information 
is being fed into your AI Agent.
</Note>

In your chatbot inside GPT-trainer, you must first create an appropriate AI Agent that you plan to give this function-calling capability to. In our example, 
...
Save the Agent. Then, head over to the “Functions” tab. Change the “Response Context Limit” to the maximum allowed. Then click “Add function”.
...
your own definition. In our case, we wrote the following:
...
Next, define the API endpoint. The Semantic Scholar endpoint we are using is: Next, define the API endpoint. The Semantic Scholar endpoint we are using is: 
https://api.semanticscholar.org/graph/v1/paper/search.


After that, choose the HTTP method. We use “Get” for our example. The specific API endpoint you are using should have documentation surrounding whether a GET 
or a POST request is appropriate.
...
Finally, we get to parameters. This is where you tell the LLM, in a structured fashion, how to provide input to the function you defined. Make sure to follow 
in the format that we provided. This information is used to guide the LLM. If you don’t set this up right, your function will not work, and you will likely get 
errors every time your Agent responds to a user query.

For our function, we define the following parameters:
...
Types in general can be “String”, “integer”, “boolean”, “array”, etc. The description is more open-ended, but also extremely important. This is where you must tell 
the LLM what this parameter represents, and if possible, provide default values and examples of what it should actually look like when the function is called.
...
Once the set of parameters have been specified, under the key field “required”, you can define which parameters are mandatory when the function is called.
