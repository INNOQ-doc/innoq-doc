...
```bash Curl
curl -X POST -F "file=@{file_path}" https://app.gpt-trainer.com/api/v1/chatbot/{uuid}/data-source/upload \
...
const apiUrl = 'https://app.gpt-trainer.com/api/v1/chatbot/{uuid}/data-source/upload';

const fileName = filePath.split('/').pop();

const fileStream = fs.createReadStream(filePath);

axios.post(apiUrl, fileStream, {
  headers: {
